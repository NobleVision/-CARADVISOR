...
| Find My Car (discovery engine) | Filters and ranks local new/used inventory against a buyer profile (min/max budget via a dual-thumb slider, location as ZIP **or** "City, ST" — resolved server-side to a ZIP, distance, mileage, body/fuel, seller type, price-vs-reliability) into a short, explained shortlist. |
...
| Inventory Map Explorer (`/map`) | Every listing pinned on a theme-aware Mapbox map (dark/light basemap follows the app theme) as a gold price pill at its real ZIP location, with popup report cards, min/max price-range, body, and risk filters, and geolocation. Buyer ZIPs outside the seeded area geocode live, so distance filtering uses real geography nationwide. |
...
| Cinematic Four-Car Hero | The landing hero uses the Car Advisor design handoff package: selectable left-column copy with evidence-first CTAs, Instrument Serif/Sans typography, atmospheric gold haze, and a self-contained `<hero-particles>` canvas loop that cycles Toyota 4Runner → Toyota Camry → Mazda CX-5 → Honda Accord from `client/public/hero/`, holds each assembled car for 3 seconds, and adds pointer/press parallax so shoppers can move the selected car. |
...
The project runs **React 19 + Tailwind 4** on the client and a **tRPC 11** API on the server, with **Drizzle ORM** over **Neon serverless Postgres** and JWT cookie sessions (demo credential login) for authentication. It deploys to **Vercel** as a Vite static site plus serverless functions (the same tRPC router runs under a local Express dev server for HMR). The UI defaults to a dark "midnight showroom" theme with Fraunces serif headings and Inter body type, with a **light/dark/system theme switcher** in the NavBar (`ThemeContext` + `ThemeToggle`; light tokens live alongside the `.dark` overrides in `index.css`).
...
| **Cloudinary** | `CLOUDINARY_URL` (single value — the SDK auto-configures from it; it embeds the key + secret) | Optimized `f_auto,q_auto` CDN delivery for all listing photos. **`pnpm sync:cloudinary`** uploads each *unique* source image once and writes the committed manifest `server/inventory/photos.cloudinary.json`; the runtime is a pure URL builder (`server/images/cloudinary.ts`) — no SDK on the request path. |
| **Mapbox** | `MAPBOX_TOKEN` (public `pk.`, URL-restrict it) | The **`/map`** inventory explorer (theme-aware dark/light basemap, gold price pins, popup report cards, filters — mapbox-gl ships in its own lazy chunk), server-side Geocoding v6 for buyer ZIPs outside the seeded centroid table, and "City, ST" → ZIP resolution for the Find My Car location field (`server/geo/cityToZip.ts` falls back to the keyless Zippopotam.us API when the token is blank). |
...
`/` is now the public brand home — a GSAP scroll story led by a design-handoff **canvas particle hero**: champagne-gold particles assemble over real duotone vehicle cutouts, respond to pointer/press parallax, and hand off into the story sections (pinned score build → the RUN stamp demo → cinematic b-roll → product tour). Everything degrades to a calm static composition under `prefers-reduced-motion`, and the landing code stays lazy-loaded so the main app bundle is untouched. The VIN lookup moved to **`/lookup`**.

**Hero refresh (June 2026):** the current landing hero now mounts the design-handoff `<hero-particles>` web component from `client/public/hero/hero-particles.js` and its sibling `cars/` metadata/PNG assets. The copy column follows the final handoff spec ("Don't buy on hope. Buy on evidence."), the primary CTA routes to `/lookup`, and the secondary CTA starts the Quick guided tour. The engine keeps reduced-motion behavior, resize handling, car-index persistence (`carAdvisorHero.carIdx`), a four-car Toyota/Mazda/Honda rotation, a 3-second assembled-car hold before each shift, and pointer/press parallax on the selected car.
...
| `server/geo/cityToZip.ts` | "City, ST" → ZIP resolution for location inputs (seeded city table → Mapbox → keyless Zippopotam.us, cached) behind `find.resolveLocation`. |
...
| `client/src/components/` | NavBar, Logo, ThemeToggle, VinSearchForm, VehicleResult, ScoreGauge, AdvisorChat, MatchCard, AdvisoryCallout, ChecklistCard, PublicRecords, SimilarVehicles, FromTheWeb, LiveMarketPanel, PremiumTeaser. |
...
| `client/src/landing/` | GSAP scroll story + landing hero shell, including the `<hero-particles>` canvas mount in `HeroSection.tsx` (lazy chunk). |
| `client/public/hero/` | Public static assets for the landing hero: `hero-particles.js`, `cars/meta.json`, and duotone car PNG cutouts used by the `<hero-particles>` custom element; the public rotation currently uses the 4Runner, Camry, CX-5, and Accord assets. |

## Testing

Run the suite with `pnpm test` (**191 tests**, all hermetic — `vitest.config.ts` blanks every service key so nothing ever touches a live API). Coverage includes VIN structural validation, the scoring engine's bounds/grade mapping and its knowledge-base integration (trap penalties, value-pick bonuses, manual-transmission waivers), the Reliability Index lookup semantics, inventory matching with Budget Buyer Mode and trust signals, the deterministic intent parser, the checklist builder, NHTSA recall fetching (mocked), env/service resolution (Z.AI fallback chain, service flags), the LLM client's URL-join + `json_schema` downgrade shim, the Pinecone client (lazy load, cache warming, timeouts) and score blending, the Cloudinary URL builder and manifest passthrough, Mapbox geocoding, city→ZIP location resolution, router-level additions (`similar`, `mapListings`, `resolveLocation`, `config.public`, onboarding persistence incl. the demo-account exemption), the Brave client (cache/throttle/429) and marketplace tagging, landing-hero asset regressions, backward compatibility for legacy saved-search criteria, and the auth flows. Known issue: the two `config.public` tests in `server/routers/config.router.test.ts` re-import the whole router graph per test and can exceed Vitest's default 5s timeout on slower machines (see [todo.md](todo.md) § Known Issues).
